...
# At the beginning of the simulation, the baton is treated as a single rigid body, and its motion is calculated through kinematic equations on its COM that include the starting speed and gravity. 
# Additionally, the rotation is calculated independently by updating the axis of orientation based on a constant angular speed set by the slider. The positions of the individual masses are
# determined by mapping them relative to the COM and current tilt of the rod.

# If the split is triggered, the simulation calculates the instanteous velocity of each mass at the moment of the split, whcih can be done by adding the vectors of the translational speed of the COM,
# the perpendicular tangential velocity caused by the rotation, and a possible impulse along the axis of orientation. After this, the masses are treated seperately and calculated using kinematic equations. 
# The only forces on the masses are gravity, and as such the simulation becomes a projectile motion calculation.
